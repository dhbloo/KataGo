...
int MainCmds::gomprotocol(const std::vector<std::string>& args) {
  ...
  // Also check loggingToStderr so that we don't duplicate the message from the log file
  cout << "MESSAGE Katagomo 2022.6.14 by HZY" << endl;
  ...
  while(getline(cin, line)) {
    ...
    if(command == "ABOUT") {
      response =
        "name=\"Katagomo\", version=\"2022.6\", author=\"HZY\", country=\"China\", email=\"2658628026@qq.com\", "
        "others=\"Based on Katago v1.7.0 by Lightvector";
    }
      ...
      if(pieces.size() == 1) {
        if(contains(pieces[0],':')) {
          ...
          if(subpieces.size() == 2 && Global::tryStringToInt(subpieces[0], newXSize) &&
      Global::tryStringToInt(subpieces[1], newYSize)) suc = true;
        }
        else {
